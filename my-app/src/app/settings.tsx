 ...
 *
 * Sections:
 *   1. Header         — dark hero, same visual language as other tabs
 *   2. Profile        — editable Name & Email with inline save/edit states
 *   3. Language       — segmented control (🇻🇳 Tiếng Việt / 🇺🇸 English)
 *                       Writing to LanguageContext updates the ENTIRE app
 *   4. Currency       — locked to VND (informational, extensible)
 *   5. App Info       — version, build, legal links
 *
  ...
  return (
    <View style={pStyles.fieldWrap}>
      ...
      {editing ? (
        ...
        <Text style={pStyles.fieldValue} numberOfLines={1}>
          {value || placeholder}
        </Text>
      )}
    </View>
...
/** Simple list row with optional right element */
function InfoRow({
  emoji,
  label,
  value,
  ...
  return (
    ...
    >
      <View style={iStyles.emojiWrap}>
        <Text style={iStyles.emoji}>{emoji}</Text>
      </View>
...
// ─── Language toggle ─────────────────────────────────────────────────────────
/**
 * Segmented two-option control for VI / EN.
 * The active segment slides to the selected option using an Animated.Value,
 * giving a smooth "pill" highlight effect built entirely from default RN components.
 */
function LanguageToggle({
  ...
  // Animated pill left position: 0% → 50% of container width
  const TOGGLE_W = 280;
  const PILL_W = TOGGLE_W / 2 - 4;
  const pillLeft = anim.interpolate({
    inputRange: [0, 1],
    outputRange: [4, TOGGLE_W / 2],
  });
  ...
  return (
    <View style={[ltStyles.track, { width: TOGGLE_W }]}>
      ...
      <Animated.View
        style={[ltStyles.pill, { left: pillLeft, width: PILL_W }]}
      />

      {/* VI option */}
      <TouchableOpacity
        style={[ltStyles.option, { width: TOGGLE_W / 2 }]}
        onPress={() => select("vi")}
        activeOpacity={0.8}
      >
        <Text
          style={[ltStyles.optText, current === "vi" && ltStyles.optTextActive]}
        >
          {viLabel}
        </Text>
      </TouchableOpacity>
      ...
      >
        <Text
          style={[ltStyles.optText, current === "en" && ltStyles.optTextActive]}
        >
          {enLabel}
        </Text>
      </TouchableOpacity>
...
const ltStyles = StyleSheet.create({
  ...
  pill: {
    ...
    top: 4,
    height: 38,
    backgroundColor: COLORS.langActive,
    borderRadius: R.xl - 2,
    zIndex: 0,
  },
  option: {
      ...
      >
        ...
        <Section title={t.settProfile}>
          ...
          <ProfileField
            label={t.settEmail}
            value={email}
            placeholder={t.settEmailPH}
            onChange={setEmail}
            editing={editing}
            keyboardType="email-address"
          />

          {/* Save changes button — only visible while editing */}
          {editing && (
            <TouchableOpacity
        ...
        <Section title={t.settPrefs}>
          ...
          <View style={styles.langBlock}>
            ...
            {/* Active language confirmation badge */}
          ...
          {/* Currency (informational, not changeable yet) */}
          <InfoRow
            emoji="💱"
            label={t.settCurrency}
            value={t.settCurrencyVal}
            last
          />
        </Section>
        ...
        <Section title={t.settAppInfo}>
          ...
          <InfoRow emoji="🔧" label={t.settBuild} value="2025.05.21" />
          <InfoRow
            emoji="🔒"
            label={t.settPrivacy}
            onPress={() => {
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            {lang === "vi"
              ? "Quản lý Tài chính Cá nhân • Phiên bản 1.0.0"
              : "Personal Finance Manager • Version 1.0.0"}
          </Text>
...
const styles = StyleSheet.create({
  ...
  // Profile summary row inside hero
  profileRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: SP.md,
